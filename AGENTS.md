...
- Barra de felicidad funcional: `services/PetHappinessService.js` con `calculatePetHappiness(current, { score, hours })` (≥90:+10, ≥75:+6, ≥60:+2, <60:−12, +2 si ≥7h, clamp 0–100). `SleepModeScreen.finishSleep` la actualiza y se persiste automáticamente en `sleep_pet_data`. `decayPetHappiness` es un no-op (cimientos para decaimiento por tiempo, pendiente).
...
- Las pantallas del BottomNav rediseñadas (Statistics, Achievements, Settings, PetShop): `NightBackground` + `SafeAreaView` + `ScrollView` + `BottomNav active` + fade-in `Animated`. Lógica intacta.
  ...
  - `SettingsScreen`: `Switch` nativo en el recordatorio + `TimeSelector` (cápsula única `[−] 19 : 15 [+]`, campos hora/minuto seleccionables por toque, botones −/+ actúan sobre el campo seleccionado con press-and-hold vía `setTimeout`/`setInterval`), caja motivacional navy, enlace "activa alarmas exactas" cuando falta el permiso, cards Reiniciar/Acerca de (`APP_VERSION` hardcode "1.0.3", sin mascota debajo).
...
- Pendiente: implementar el decaimiento por tiempo de la felicidad (`decayPetHappiness`, ya esbozado); las pantallas ya están todas rediseñadas y sin emojis en la UI (iconos vía `AppIcon`).
