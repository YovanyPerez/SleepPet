...
- **`versionCode` se define en `app.json` (`android.versionCode: 4`)** — `expo prebuild` lo resetea en `build.gradle` si no está ahí (por eso "paquete no válido" al instalar sobre una versión con `versionCode` mayor). `version` en `app.json`/`package.json` = "1.0.4". `adb install -r` sobre `1.0.3/3` mantiene `AsyncStorage`/`SharedPreferences` (racha/historial/mascotas); `adb uninstall` borra todo.
